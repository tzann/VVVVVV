...
void Graphics::drawentity(const int i, const int yoff)
{
    ...
    const int player_xp = lerp(obj.entities[0].lerpoldxp, obj.entities[0].xp);
    const int player_yp = lerp(obj.entities[0].lerpoldyp, obj.entities[0].yp) - yoff;

    const int xp = lerp(obj.entities[i].lerpoldxp, obj.entities[i].xp) - player_xp + 160;
    const int yp = lerp(obj.entities[i].lerpoldyp, obj.entities[i].yp) - player_yp + 120;
...
{
    // always redraw tiles
    if (true || !foregrounddrawn)
    {
        ClearSurface(foregroundBuffer);
        if(map.tileset==0)
        {
            for (int j = -30; j < 30 * 2; j++)
            {
        ...
        {
            for (int jt = -30; jt < 30 * 2; jt++)
            {
                for (int it = -40; it < 40 * 2; it++)
                {
...
void Graphics::drawfinalmap(void)
{
    ...
    if (true || !foregrounddrawn) {
        ...
        if(map.tileset==0){
            ...
            for (int j = -30; j < 2 * 30; j++) {
                for (int i = -40; i < 2 * 40; i++) {
                    if((map.contents[TILE_IDX(i, j)])>0)
...
void Graphics::drawtowermap(void)
{
    ...
    for (int j = 0; j < 31; j++)
    {
        for (int i = 0; i < 40; i++)
        {
            ...
            if (temp > 0) {
                drawtile3(i * 8 - player_xp + 160, (j * 8) - (yoff % 8) - player_yp + 120, temp, towerbg.colstate);
                if (isInWrappingAreaOfTower) {
                    drawtile3((i - 40) * 8 - player_xp + 160, (j * 8) - (yoff % 8) - player_yp + 120, temp, towerbg.colstate);
                    drawtile3((i + 40) * 8 - player_xp + 160, (j * 8) - (yoff % 8) - player_yp + 120, temp, towerbg.colstate);
                }
            }
        }
...
void Graphics::drawtowerspikes(void)
{
    ...
    for (int i = 0; i < 40; i++)
    {
        drawtile3(i * 8 - player_xp + 160, -8+spikeleveltop - player_yp + 120, 9, towerbg.colstate);
        drawtile3(i * 8 - player_xp + 160, 230-spikelevelbottom - player_yp + 120, 8, towerbg.colstate, 8 - spikelevelbottom);
        if (isInWrappingAreaOfTower) {
            drawtile3((i - 40) * 8 - player_xp + 160, -8 + spikeleveltop - player_yp + 120, 9, towerbg.colstate);
            drawtile3((i + 40) * 8 - player_xp + 160, 230 - spikelevelbottom - player_yp + 120, 8, towerbg.colstate, 8 - spikelevelbottom);
        }
    }
}
...
void Graphics::drawforetile(int x, int y, int t)
{
    ...
    SDL_Rect rect;
    setRect(rect, x - player_xp + 160,y - player_yp + 120,tiles_rect.w, tiles_rect.h);
